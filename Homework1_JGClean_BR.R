...
join <-
  st_join(tracts12B, MBTAStationBuffers) %>%
...
popbystation <-
  ...
  st_drop_geometry() %>%
  left_join(MBTASTOPS_CUT)
 
MBTASTOPS_POP <- dplyr::left_join(MBTASTOPS_CUT, popbystation)

ggplot() + 
  geom_sf(data= st_union(tracts12B), fill="black") +
  geom_sf(data= MBTASTOPS_POP, aes(size = sumPop), color="red")
...
rentbystation <-
  join %>%
  group_by(STATION) %>%
  summarize(avgRent = mean(MedRent)) %>%
  st_drop_geometry() %>%
  left_join(MBTASTOPS_CUT)

MBTASTOPS_RENT <- dplyr::left_join(MBTASTOPS_CUT, rentbystation)

ggplot() + 
  geom_sf(data= st_union(tracts12B), fill="black") +
  geom_sf(data= MBTASTOPS_RENT, aes(size = avgRent), color="red")
  labs(title = "Percent White 2012-2018", subtitle = "%") +
